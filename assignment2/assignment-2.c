...
void usd_to_eur(void)
{
	...
	cents = discrete_cents / 100;
	dollars += cents;
	dollars *= TOEUROS;
	printf("\nEUR value is: %lf", dollars);
}
...
void eur_to_usd(void)
{
	...
	real_cents = (real_dollars - dollars) * 100;
...
void fahr_to_cels(void)
{
	...
	scanf_s("%d", &fahr);
	celsius = (double)((TOCELS)*((double)fahr - 32));
...
void cels_to_fahr(void)
{
	...
	scanf_s("%lf", &celsius);
	printf("%lf", celsius);
	real_fahr = (double)((celsius*(TOFAHR)) + 32);
...
void lbs_to_kg(void)
{
	...
	scanf_s("%d %d", &lbs, &oz);
	oz_real = ((double) oz) / 16;
	lbs_real = lbs + oz_real;
	kg = (lbs_real*453.59237) / 1000;
